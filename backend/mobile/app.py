...
def start():
    ...
    difficulty.append(items[item_index, 1])
    return administered_items


@app.route('/CAT/<res>', methods=['POST'])
def CAT(res):
    global items, administered_items, responses, est_theta, flag, item_index, theta_list, difficulty
    ...
    est_theta = estimator.estimate(
        items=items, administered_items=administered_items, response_vector=responses, est_theta=est_theta)
    ...
    if _stop:
        ...
        item_index = selector.select(
            items=items, administered_items=administered_items, est_theta=est_theta)
...
def result():
    ...
    print(new_value)
    res = round(new_value)
    return jsonify({"proficiency": res})
...
def graph():
    global items
    output = io.BytesIO()
    catplot.gen3d_dataset_scatter(items, 'Scatter', show=False)
